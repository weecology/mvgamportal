...
fit_models = function(data,train_starts,type){
  ...
  for (i in seq_along(train_starts)){
    ...
    #intercept_prior <- prior(std_normal(), class = Intercept)
...
in_train_starts = c(299) 

fit_models(data_all, in_train_starts, "in")

# with our set priors
baseline_model <-readRDS("case_study/model_outputs/baseline_inregime_output375.rds")
model_ar <- readRDS("case_study/model_outputs/ar_inregime_output375.rds")
plot(baseline_model, type = 'forecast', series = 5)
plot(model_ar, type = 'forecast', series = 5)
summary(baseline_model) 
model_ar$prior

#fuzzy caterpillar plots
plot

# mvgam default priors
default_baseline_model <-readRDS("case_study/model_outputs/default_priors_baseline_inregime_output375.rds")
default_model_ar <- readRDS("case_study/model_outputs/default_priors_ar_inregime_output375.rds")
plot(default_baseline_model, type = 'forecast', series = 1)
plot(default_model_ar, type = 'forecast', series = 1)

#with formula y ~ -1
# minus1baseline_model <-readRDS("case_study/model_outputs/minus1baseline_inregime_output375.rds")
# minus1model_ar <- readRDS("case_study/model_outputs/minus1ar_inregime_output375.rds")
# plot(minus1baseline_model, type = 'forecast', series = 2)
# plot(minus1model_ar, type = 'forecast', series = 2)
# minus1baseline_model$prior
# minus1model_ar$prior


# further looking at priors
baseline_model$prior # this code pulls priors from set model
model_ar$prior

# getting data train outside of function because of error
gap <- 16
train_start <- 299
train_stop  <- train_start + 60 - 1
test_start  <- train_stop + gap + 1
test_stop   <- test_start + 7 - 1
...
#getting the default priors from get_mvgam_priors()
priors_default <- get_mvgam_priors(
  formula = y ~ series,
  trend_model = AR(),
  family = poisson(),
  data = data_train,
...
priors_default # produces a table with intercept, series, ar1, and sigma info
