...
#' 
#' 
get_regime <- function(data, regime = 4, test = "out") {
 ...
 if(test == "in") {         
  ...
  train <- data %>%
           dplyr::filter(newmoonnumber >= train_start,
                         newmoonnumber <= train_stop)
  test <- data %>%
          dplyr::filter(newmoonnumber >= test_start,
                        newmoonnumber <= test_stop)
  return(list(regime=regime,train=train,test=test))
}
